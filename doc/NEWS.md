...
pgBunker can be installed from the [PostgreSQL Yum Repository](https://yum.postgresql.org) or the [PostgreSQL APT Repository](https://apt.postgresql.org) and packages are also available for many other distributions. Source code can be downloaded from [releases](https://github.com/pgbunker/pgbunker/releases).
...
- Per our policy to support five EOL versions of PostgreSQL, 9.5 is no longer supported by pgBunker.
